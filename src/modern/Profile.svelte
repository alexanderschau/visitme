<script lang="ts">
	...
	import type { Profile, ColorList } from '../types';
	...
	let colors: ColorList = getColors({
		...
		<Edit {edit} bind:profile on:save={(e) => dispatch('save', e.detail)}>
			<div class="profile">
				{#if edit}
					<ImageWrapper
						on:change={async (e) => {
							...
							profile.img = base64;
							profile.primaryColor = [averageColor.r, averageColor.g, averageColor.b];
						}}><img class="img" alt="profile-img" src={profile.img} /></ImageWrapper
					>
				{:else}
				...
				<div class="profileInfo">
					<div class="name">
						{#if edit}<input type="text" class="inpt" bind:value={profile.name} />{:else}
							{profile.name}
						{/if}
					</div>
					<div class="description">
						{#if edit}<input type="text" class="inpt" bind:value={profile.description} />{:else}
							{profile.description}
						{/if}
					</div>
				</div>
				{#if edit}
					<LinkWrapper let:link linkList={profile.links} let:linkId={i} let:active>
						<div
							class="link"
